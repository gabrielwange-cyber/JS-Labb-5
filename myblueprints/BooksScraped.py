...
def buttonchoice():
    button_picked = request.form.get('show_all')  
    if button_picked == 'show_all':
        buttonchoice == 'show_all_TRUE'
        return render_template('Home_Page.html')

@BooksScraped.route('/remove', methods=['GET'])
def remove_json():
    return render_template('Remove.html')

@BooksScraped.route('/removejson', methods=['DELETE','POST'])   
def remove_json_file():
    category_name = request.form.get('category')
    book_name = request.form.get('book')
    
    if not category_name or not book_name:
        return jsonify({"error": "category and book are required"}), 400
    
   
    category_filename = f"./JsonData/{category_name}.json_{currenttime}.json"
    if not os.path.exists(category_filename):
        return jsonify({"error": "category file missing"}), 404
    
    try:
        with open(category_filename, 'r', encoding='utf-8') as fp:
            category_data = json.load(fp)
            original_count = len(category_data.get('books', []))
            category_data['books'] = [book for book in category_data.get('books', []) 
                                      if book['title'] != book_name]
            removed_count = original_count - len(category_data['books'])
        
        
        with open(category_filename, 'w', encoding='utf-8') as fp:
            json.dump(category_data, fp, ensure_ascii=False, indent=4)
        

        master_path = f'{Full_book_list}_{currenttime}.json'
                ...
                with open(master_path, 'r', encoding='utf-8') as fp:
                    ...
                    for cat in master_data:
                        if cat['category'] == category_name:
                            cat['books'] = [book for book in cat.get('books', []) 
                                          if book['title'] != book_name]
                            break
        ...
        return jsonify({"message": f"Removed {removed_count} book(s) '{book_name}' from '{category_name}'"}), 200
    
    except Exception as e:
        print(f'Error removing book: {e}')
        return jsonify({"error": f"failed to remove book: {str(e)}"}), 500


@BooksScraped.route('/PUT', methods=['GET', 'POST'])
def put_json():
    return render_template('PUT.html')

@BooksScraped.route('/putjson', methods=['PUT','POST'])
def put_json_file():
    title_name = request.form.get('new_title')
    _newcategory = request.form.get('new_cata')
    rating = request.form.get('new_rating')
    tumbnail = request.form.get('new_tumbnail')
    price_gdp = request.form.get('new_price_gdp')
    price_sek = request.form.get('new_price_sek')

    Full_book_list = _newcategory

    if not title_name and category and rating and tumbnail and price_gdp and price_sek:
        return jsonify({"Error: Invalid request"})
    
    category = f"./JsonData/{category}.json_{currenttime}.json"
    if not os.path.exists(category):
         with open(f'{Full_book_list}_{currenttime}.json', 'w', encoding='utf-8') as file_pointer_new:
            json.dump(category)

@BooksScraped.route('/BooksinCatagory', methods=['GET', 'POST'])
def books_by_category():
    category_name = request.form.get('category')  # Get the category selected in the form
    if not category_name:
        # if nothing was sent we return an error message
        return jsonify({}), 400

    filename = f"./JsonData/{category_name}.json_{currenttime}.json"

    if not os.path.exists(filename):
        return jsonify({"error": "category file missing"}), 404

    with open(filename, 'r', encoding='utf-8') as file_pointer_category:
        category_data = json.load(file_pointer_category)
        return jsonify(category_data)


@BooksScraped.route('/categories', methods=['GET'])
def All_catagories():
    Catagories = f'{Full_book_list}_{currenttime}.json'
    
    category_list = []
    
    if os.path.exists(Catagories):
        try:
            with open(Catagories, 'r', encoding='utf-8') as fp:
                all_categories = json.load(fp)
    ...
    # fallback: read category names from the individual .json files in JsonData
    try:
        json_dir = './JsonData'
        if os.path.exists(json_dir):
            for filename in os.listdir(json_dir):
                if filename.endswith(f'_{currenttime}.json'):
                    # extract category name from filename like "Fantasy.json_2026-02-26.json"
                    category_name = filename.replace('.json_' + currenttime + '.json', '')
                    if category_name:
                        category_list.append(category_name)
            category_list.sort()  # sort alphabetically
            return jsonify(category_list)
    except Exception as e:
        print(f'Error reading category files: {e}')
    
    return jsonify(category_list)
@BooksScraped.route('/show_all')
def Books_all():
    return render_template('Show_all.html', methods = ['GET'])


@BooksScraped.route('/showall', methods=['GET'])
def Showing_all_books():
    Catagories = f'{Full_book_list}_{currenttime}.json'
    if os.path.exists(Catagories):
        try:
            with open(Catagories, 'r', encoding='utf-8') as fp:
